...
actuation with cloud and edge processing. As such, our running example in
...
\sclist{0.58}{%
  \lstinputlisting[language=Sancus-C]{mflos.c}}{A translation of module \module{FloS1} to C}%
...
in the application binary loaded in the Sancus microcontroller. At startup,
these modules are immediately initialized to gain exclusive access to their I/O
devices (\ref{ass:startup}). To change this behavior, an attacker would require
physical access to the microcontroller in order to upload a malicious binary;
however, this is out of scope (\cref{concept:attacker}).
...
\sclist{0.5}%
  {\lstinputlisting[language=rust]{mflos.rs}}{A translation of module \module{FloS1} to Rust}%
...
\sclist{0.7}{\maxsizebox{.99\linewidth}{!}%
  ...
  {Remote attestation process on ARM TrustZone with OP-TEE}%
...
\emph{fTPM}~\cite{raj2016ftpm}) that provides security guarantees similar to a hardware
...
\subsection{Comparison: SMs on Different TEEs}
...
\sclist{0.5}%
  ...
  {Excerpt from a deployment descriptor in YAML format}%
