...
\begin{figure}[t]
  \hspace*{-0.3cm}
  \centering
  \includegraphics[width=0.9\columnwidth]{graphics/smart-home.drawio.pdf}

  \caption[Setup of our \emph{smart home} application]{Setup of our \emph{smart home} application. Each \ac{SM} is deployed
...
\begin{figure*}[t]
  ...
  \end{subfigure}
  \caption[Average encryption timings on different TEEs]{Average timings to perform encryption using \aes-128 and
...
\cref{fig:eval-crypto} shows the average time calculated on each \ac{TEE} to
...
protection, running on the Intel \ac{SGX} node (\emph{Native x86} in the
figure). The plots show average timings computed over 110 iterations, except
that Sancus values have been extrapolated from previous
experiments~\cite{noorman_sancus2}.
...
\begin{sidewaysfigure}
  \resizebox{0.92\linewidth}{!}{\input{microb_sequence.tex}} \caption[Control flow and timings of the smart home application]{Sequence
...
cryptographic operations, which may be optimized in future releases of OP-TEE.
On the other hand, \spongent{} operations performed in software caused huge
performance penalties on the application, taking a total of 208.62 ms
($\sim$48.2\% of the \ac{RTT}).
...
channel between the Sancus node and the deployer, as explained above. The
deployment process for Intel \ac{SGX} and TrustZone takes only 247 ms and 438
ms, respectively; Binary sizes are shown in \cref{tbl:scenario_tcb}.
